...
/** move this part in a separate source code file: ident05_collinput
   ...
   // in the case of Initial Value Problem, boundry[0] is the state and boundry[1] is the derivative
     ...
     if (type_predict==COL_TYP_BVP_PRED_TRICKED) {
	  ...
	  boundary_all[2*(k)+1]=predictparams[0]*sin(2*3.14156*t/predictparams[1]);
		 ...
		 else if (repeat_predict==COL_TYP_BVP_GUESSED {
			//t=times_end[k];
			// complete here solution
			//boundary_all[2*(k)]==yguessed[2*num_points*(k-1)+num_points-1][1];
			//t=time_end[k+1];
			//boundary_all[2*(k)+1]==yguessed[2*num_points*(k)+num_points-1][1];
