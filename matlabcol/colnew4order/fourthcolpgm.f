...
C     THE FOURTH ORDER LINEARIZED SYSTEM IS AS FOLLOWED
C     X1d=X2     
C     X2d=(DET^(-1))*{J22*(-m1*g*l1/2-m2*g*l1-m2*g*l2/2)*X1 +
C     J12*(m2*g*l2/2*X1) + J12*(m2*g*l2/2*(X2)}     
C     X3d=X4
C     X4d=(DET^(-1))*{J21*(-m1*g*l1/2-m2*g*l1-m2*g*l2/2)*X1) + J11*(m2*g*l2/2*X1)
C     + J11*(m2*g*l2/2*X2)}
C   WHERE:
C     m1=1.
C     m2=0.25
...
C          J22=0.0092
C          DETJ=0.000285
C          -m1*g*l1/2-m2*g*l1-m2*g*l2/2  = -4.495
C          -m2*g*l2/2 = -0.817 
C          -m2*g*l2/2 = -0.408
C in all          
C     X1d= X2
C     X2d= -71.8*X1 +45.3*X3
C     X3d= X4
C     X4d= 190*X1-196*X3
      ...
      REAL,POINTER :: F(:)
      F(1)=ZVAL(2)
      F(2)=-71.8*ZVAL(1) +45.3*ZVAL(3)
      F(3)=ZVAL(4)
      F(4)=190.0*ZVAL(1)-196.0*ZVAL(3)
      END SUBROUTINE FSUB
...
C      REAL,DIMENSION(:) :: DF
      ...
      IZETA(1)=1
      IZETA(2)=12
      GVAL(1)=(ZVAL(1)-1.0)**2+(ZVAL(3)-0.0)**2
      GVAL(2)=(ZVAL(1)+1.0)**2+(ZVAL(3)-0.0)**2
      END SUBROUTINE GSUB
...
C      ALLOCATE(XII(K*N))
C     DO I=0,K*N-1
C         XII(I)=I/(K*N-1)*3.1457*2/70.0
C     ENDDO
      ZVAL(1)=COS(XII(1)*70.0)
      ZVAL(2)=-70.0*SIN(XII(1))
      ZVAL(3)=SIN(XII(1)*70.0)
      ZVAL(4)=70.0*COS(XII(1))
      DMVAL(1)=-71.8*ZVAL(1)+45.3*ZVAL(3)
      DMVAL(2)=190.0*ZVAL(1)-196.0*ZVAL(3)
      END SUBROUTINE GUESS
C  MAIN PROGRAM:

      PROGRAM FOURTHORDERPGM
