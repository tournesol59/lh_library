...
*  given floating-point and integer workspace.
      ...
      isp = lwrkfl - 2*ntol - 9*ncomp - 5*ncomp*ncomp
  ...
  901 format(1h ,'nmax from workspace =',i8)
      nmax = min(nmax, nucol)
      if (iprint .ge. 0 .and. nmax .eq. nucol) write(8,902) nmax
  902 format(1h ,'nmax', i8)
...
*  to u and tol.
     ...
     *             nmold, xxold, maxmsh) 
         ...
         if (.not. maxmsh)  call initu(ncomp, nmsh, nudim, u)
...
*  6th order deferred correction.  Then, for a nonlinear problem,
...
*  exit from blkdcm.
      ...
      if (iflfac .eq. (-1)) then
         if (iprint .ge. 0) write(6,905) iter
         iflag = -1 
         return
      endif
...
*  points for component icmp into a single vector tmwork.
     ...
     *                 sumrhs, intref)
         ...
         if (pdebug) write(6,902) icmp, tstval, rbigst, rsecnd
