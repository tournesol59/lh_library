...
int main() {
  ...
  char impFileName[15];
  strncpy(impFileName, "discmodeldaten", 15);  // sizeof: p+1 is sure

  // select type of orig. data: slope(0) or AR1(1)
  int dselect;
  std::cout << "Please select random source source (0: slope, 1: AR(1), not <2: from file>)";
  std::cin >> dselect;
  // zero ctl
  for (k=0; k<Npty; k++) {
  ...
  // between -0.5 and 0.5, (slope 0.2 to test the variance of the Cpp random generator (see line 57)
  if (dselect==0) {
     generate_rdom_example(outdata, Npty, 0.8, 0.2, 0.5);
  } else if (dselect==1) {
     ar_params.push_back(0.875);
     generate_arN_example(outdata, Npty, 1, 1.0, ar_params, 0.5);
  } else if (dselect==2) {
     std::cout << "File: " << impFileName << "\n";
     generate_from_file(outdata, Npty, impFileName);
  }
